...
{
	return unsigned(m_asm->deposit()) - _baseOffset - 1;
}

unsigned CompilerContext::currentToBaseStackOffset(unsigned _offset) const
{
	return unsigned(m_asm->deposit()) - _offset - 1;
}
...
)
{
	...
	)
	{
		...
		auto stackDepth = static_cast<size_t>(_localVariables.end() - it);
		...
		if (_context == yul::IdentifierContext::RValue)
			_assembly.appendInstruction(dupInstruction(static_cast<size_t>(stackDiff)));
		else
		{
